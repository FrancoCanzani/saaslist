...
export function AllProductsContent({
  ...
  const createPageUrl = (newPage: number) => {
    ...
    return `/browse/all${queryString ? `?${queryString}` : ""}`;
  };

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-medium mb-2">All Products</h1>
        <p className="text-muted-foreground text-sm">
          Browse all {filteredAndSortedProducts.length} bootstrapped SaaS tools
        </p>
      </div>
            ...
            <Pagination>
              <PaginationContent>
                <PaginationItem>
                  {searchParams.page > 1 ? (
                    <PaginationPrevious
                      href={createPageUrl(searchParams.page - 1)}
                    />
                  ) : (
                ...
                {(() => {
                  ...
                  let start = Math.max(
                    1,
                    searchParams.page - Math.floor(maxVisible / 2),
                  );
