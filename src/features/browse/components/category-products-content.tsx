...
export function CategoryProductsContent({
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (searchParams.tag && searchParams.tag.trim()) {
      products = products.filter((product) =>
        product.tags?.some(
          (productTag) => productTag.toLowerCase() === searchParams.tag.toLowerCase(),
        ),
      );
    }
    ...
    return products;
  }, [allProducts, searchParams.search, searchParams.sort, searchParams.tag, fuse]);
  ...
  const handleTagClick = (tag: string) => {
    const newTag = searchParams.tag === tag ? null : tag;
    setSearchParams({
      tag: newTag || null,
      page: 1,
    });
  ...
  const createPageUrl = (newPage: number) => {
    ...
    return `/browse/category/${category.slug}${queryString ? `?${queryString}` : ""}`;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {category.tags.map((categoryTag) => (
          <Badge
            key={categoryTag}
            variant={searchParams.tag === categoryTag ? "default" : "secondary"}
            className="cursor-pointer"
            onClick={() => handleTagClick(categoryTag)}
          >
            {categoryTag}
            {searchParams.tag === categoryTag && (
              <X
